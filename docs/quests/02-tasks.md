...
**Scope.** Store rooted at `<qm-home>/quests/<id>.html` (`<qm-home>` = `~/.questmaster`), never a repo. `qm quest new` scaffolds a `wip` quest with an auto-generated id. `qm quest ls` lists. `qm quest view <id>` prints the T2 detail render. `qm quest open <id>` runs T3 and opens the browser. `qm quest edit <id>` opens the extracted JSON in `$EDITOR`, then validates (T1) and rebuilds (T3) on save, refusing malformed.
**Depends.** T1, T2, T3.
