...
func TestUpdateOpenCodeIdentitySerializesPublicationAfterStateWrite(t *testing.T) {
	root := setStateRoot(t)
	s, err := NewStore(root)
	if err != nil {
		t.Fatalf("new store: %v", err)
	}
	const id = "qm-opencode-identity-lock"
	if err := s.Create(Manifest{SessionID: id, Agents: []AgentManifest{{Name: "opencode", ResumeID: "old"}}}); err != nil {
		t.Fatalf("create: %v", err)
	}

	mutate := func(value string) func(*Manifest, *SessionState) bool {
		return func(m *Manifest, ss *SessionState) bool {
			pane := ss.Panes["primary"]
			pane.OpenCodeSessionID = value
			ss.Panes["primary"] = pane
			m.Agents[0].ResumeID = value
			m.SetExtra("opencode_session_id", value)
			return true
		}
	}
	publishedA := make(chan struct{})
	releaseA := make(chan struct{})
	doneA := make(chan error, 1)
	go func() {
		_, err := s.UpdateOpenCodeIdentity(id, StateEvent{Action: "session.updated"}, mutate("ses_a"), func() error {
			close(publishedA)
			<-releaseA
			return nil
		})
	...
	mutatedB := make(chan struct{})
	publishedB := make(chan struct{})
	doneB := make(chan error, 1)
	go func() {
		_, err := s.UpdateOpenCodeIdentity(id, StateEvent{Action: "session.updated"}, func(m *Manifest, ss *SessionState) bool {
			close(mutatedB)
			return mutate("ses_b")(m, ss)
		}, func() error {
			close(publishedB)
			return nil
		})
	...
	select {
	case <-mutatedB:
		t.Fatal("B mutated while A still held the manifest publication lock")
	case <-time.After(50 * time.Millisecond):
	}
	select {
	case <-publishedB:
		t.Fatal("B published while A still held the manifest publication lock")
	case <-time.After(50 * time.Millisecond):
	}
	close(releaseA)
	...
	if ss.Panes["primary"].OpenCodeSessionID != "ses_b" || manifest.ExtraString("opencode_session_id") != "ses_b" || manifest.Agents[0].ResumeID != "ses_b" {
		t.Fatalf("final state/manifest = %+v / %+v", ss.Panes["primary"], manifest)
	}
	logPath := SessionStateLogPath(root, id)
	before, err := os.ReadFile(logPath)
	if err != nil {
		t.Fatalf("read accepted event log: %v", err)
	}
	accepted, err := s.UpdateOpenCodeIdentity(id, StateEvent{Action: "session.updated"}, func(*Manifest, *SessionState) bool {
		return false
	}, func() error {
		t.Fatal("rejected identity published side effects")
		return nil
	})
	if err != nil || accepted {
		t.Fatalf("rejected identity = accepted:%t err:%v", accepted, err)
	}
	after, err := os.ReadFile(logPath)
	if err != nil {
		t.Fatalf("read rejected event log: %v", err)
	}
	if string(after) != string(before) {
		t.Fatal("rejected identity appended an event")
	}
}

func TestUpdateOpenCodeIdentityRollsBackStateWhenManifestWriteFails(t *testing.T) {
	root := setStateRoot(t)
	s, err := NewStore(root)
	if err != nil {
		t.Fatalf("new store: %v", err)
	}
	const id = "qm-opencode-identity-rollback"
	manifest := Manifest{SessionID: id, Agents: []AgentManifest{{Name: "opencode", ResumeID: "ses-old"}}}
	manifest.SetExtra("opencode_session_id", "ses-old")
	if err := s.Create(manifest); err != nil {
		t.Fatalf("create: %v", err)
	}
	if err := SaveSessionState(id, &SessionState{SessionID: id, Version: SchemaVersion, Panes: map[string]PaneState{
		"primary": {OpenCodeSessionID: "ses-old", OpenCodeSessionUpdateSeq: 41, Recent: []string{"old"}},
	}}); err != nil {
	...
	mutate := func(m *Manifest, ss *SessionState) bool {
		pane := ss.Panes["primary"]
		if pane.OpenCodeSessionUpdateSeq >= 42 {
			return false
		}
		pane.OpenCodeSessionID = "ses-new"
		pane.OpenCodeSessionUpdateSeq = 42
		pane.Recent[0] = "new"
		ss.Panes["primary"] = pane
		m.Agents[0].ResumeID = "ses-new"
		m.SetExtra("opencode_session_id", "ses-new")
		if blockedManifest {
			if err := os.Chmod(root, 0o500); err != nil {
				t.Fatalf("block manifest write: %v", err)
			}
		}
		return true
	}
	...
	published := false
	ev := StateEvent{Action: "session.updated"}
	accepted, err := s.UpdateOpenCodeIdentity(id, ev, mutate, func() error {
		published = true
		return nil
	})
	if err == nil || accepted || published {
		t.Fatalf("failed update = accepted:%t published:%t err:%v", accepted, published, err)
	}
	if err := os.Chmod(root, 0o700); err != nil {
		t.Fatalf("restore manifest write: %v", err)
	}
	...
	pane := ss.Panes["primary"]
	if pane.OpenCodeSessionID != "ses-old" || pane.OpenCodeSessionUpdateSeq != 41 || pane.Recent[0] != "old" || manifest.Agents[0].ResumeID != "ses-old" || manifest.ExtraString("opencode_session_id") != "ses-old" {
		t.Fatalf("failed update survived: state=%+v manifest=%+v", pane, manifest)
	}
	...
	if err != nil || !accepted || !published {
		t.Fatalf("retry = accepted:%t published:%t err:%v", accepted, published, err)
	}
	ss, err = LoadSessionState(id)
	if err != nil {
	...
	manifest, err = s.Read(id)
	if err != nil {
		t.Fatalf("read retried manifest: %v", err)
	}
	pane = ss.Panes["primary"]
	if pane.OpenCodeSessionID != "ses-new" || pane.OpenCodeSessionUpdateSeq != 42 || pane.Recent[0] != "new" || manifest.Agents[0].ResumeID != "ses-new" || manifest.ExtraString("opencode_session_id") != "ses-new" {
		t.Fatalf("retry did not advance state/manifest: state=%+v manifest=%+v", pane, manifest)
	}
}
