...
const artifactPromptGuide = `When you create an HTML file, Markdown report, image, or artifact that should be visible to the user, write it under Questmaster-owned artifact storage: $QUESTMASTER_STATE_ROOT/artifacts/projects/<project-slug>/ (or ~/.questmaster-state/artifacts/projects/<project-slug>/ when unset). Use YYYY-MM-DD-<slug>.<ext> for single artifacts/reports; use YYYY-MM-DD-<slug>/ for multi-file bundles and keep related files under it, such as tasks/*.md. For HTML reports, preserve docs metadata such as docs-title, docs-date, and docs-project where practical. Then register the created or updated primary artifact path with questmaster artifact add /absolute/path/to/file.md --label "Readable title". The artifact kind is inferred from the extension; use --kind only when you need an override. If you edit the same file later, run artifact add again for the same path; artifact add updates the existing path-keyed entry, and the viewer live-reloads selected files. Use questmaster artifact rm <path-or-index> only when the artifact should leave the session list.`
