...
(2) Spawn workers with questmaster spawn [--primary <agent>] [--prompt "..."] [title]. Workers default to your own agent — omit --primary unless a task clearly fits another harness better. Harness guide: claude is strong for general-purpose coding and orchestration; codex for reliable general coding with sandboxed execution; pi is lightweight and fast for small, well-scoped tasks; omp has a built-in LSP and an interactive debugger (set breakpoints, step, inspect variables, evaluate expressions), so prefer omp for bug-hunting and root-causing crashes. A chosen harness that is not installed fails the spawn with a clear error — fall back to your own agent. Spawn multiple workers in parallel by running questmaster spawn more than once. Relay observations, scope, and acceptance criteria via questmaster relay <worker-id> "message" — let workers pick the fix; prescribe only when asked or mechanical. Broadcast to all workers with questmaster broadcast "message", inspect workers with questmaster workers or questmaster read <worker-id>, and require workers to report back via questmaster report from the worker session.
