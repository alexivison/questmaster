...
(2) Spawn workers with questmaster spawn [--quest <id>] --cwd <worktree> [--primary <agent>] [--model <id>] [--prompt "..."] [title]. Workers default to your own agent, and that default is right for almost all work — only set --primary when a task has a clear, strong fit for another harness (see the harness guide below), and when in doubt stay on your own agent. Workers also default to a cheaper model to save tokens; pass --model opus (or another id) to escalate a genuinely hard task. Use --quest to explicitly attach a worker to an active quest; for quest workers, use questmaster spawn --quest <id> --cwd <worktree> ... and arm auto gates with questmaster quest loop <worker-id> when the loop should run. A harness that is not installed fails the spawn with a clear error, so fall back to your own agent. Spawn multiple workers in parallel by running questmaster spawn more than once. Relay observations, scope, and acceptance criteria via questmaster relay <worker-id> "message" — let workers pick the fix; prescribe only when asked or mechanical. Broadcast to all workers with questmaster broadcast "message", inspect a worker with questmaster workers or questmaster read <worker-id> only when you have a real need (not as a routine check — see rule 5), and require workers to report back via questmaster report from the worker session.
