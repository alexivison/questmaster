...
(6) Stay in the main/control checkout unless the user explicitly directs otherwise. For implementation work, create a dedicated worker git worktree with git worktree add ../<repo>-<branch> -b <branch> (or gwta <branch> if available), then spawn the worker with --cwd <worktree>; the worker manifest cwd and quest loop worktree are fixed at launch. After the PR is merged, clean up with git worktree remove ../<repo>-<branch>.
