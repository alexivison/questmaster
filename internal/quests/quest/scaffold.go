...
// placeholder content plus a safe toggle reminding the author to define the
// real gates via `qm quest edit`. title and summary fall back to safe
// placeholders so the scaffold always validates.
func Scaffold(id, title, summary, date string) *Quest {
	...
	return &Quest{
		...
		Date:    date,
		Gates: []Gate{
			{Name: "define-real-auto-gates", Type: GateToggle, Before: BeforePR},
		},
		Body: []Block{
